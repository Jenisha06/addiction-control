...
export default function DashboardPage() {
  ...
  const levelProgress = useMemo(() => {
    ...
    return Math.min(
      100,
      Math.max(0, ((userData.xp - currentLevelXP) / (nextLevelXP - currentLevelXP)) * 100)
    );
  }, [userData]);
  ...
  const streak = useMemo(() => {
    ...
    for (let checkin of sorted) {
      ...
      if (diff === 0 || diff === 1) { count++; current = checkDate; }
      else break;
    }
    return count;
  }, [checkins]);
  ...
  const moneySaved = useMemo(() => {
    if (!userData?.sobrietyDate || !userData?.drinkCostPerDay) return 0;
    const days = (Date.now() - new Date(userData.sobrietyDate)) / (1000 * 60 * 60 * 24);
  ...
  const chartData = useMemo(() => {
    return checkins.map((c) => ({
      name: c.date.toDate().toLocaleDateString("en-US", { weekday: "short" }),
      mood: c.mood,
      cravings: c.cravings,
    }));
  }, [checkins]);
  ...
  if (!userData) return <div className="p-10 text-center text-red-500 font-bold">No user data found.</div>;
  ...
  return (
    <>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="p-6 max-w-lg mx-auto space-y-8 pb-32" // pb-32 keeps content above the nav
      >
        {/* HEADER */}
        <header className="flex justify-between items-start pt-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-bold text-slate-800">Ready for today?</h1>
        ...
        <section className="bg-white rounded-3xl p-8 shadow-sm border text-center space-y-4">
          <p className="text-xs uppercase text-slate-400">Alcohol Free For</p>
...
function DashboardCard({ title, icon, onClick }) {
  return (
    <button onClick={onClick} className="bg-slate-50 p-6 rounded-3xl text-left hover:scale-105 transition">
